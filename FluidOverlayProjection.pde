...
SyphonServer server;
  ...
  private class MyFluidData implements DwFluid2D.FluidData{
    ...
    public void update(DwFluid2D fluid) {
      ...
      vx     = (px - mx[index]) * +vscale;
      vy     = (py - my[index]) * -vscale; 
      fluid.addVelocity(px, py, 15, vx, vy);
...
//      pg_tex_handle[0] = pg.getTexture().glName;
  ...
  public void setup() {
    ...
    (mv = new Movie(this, "Desktop/1.mp4") ).loop();
    while (mv.width == 0){
      delay(0);
      mv.get(width,height);
    }
    ...
    imageOverlay = createGraphics(viewport_x,viewport_y,P3D);

    kinect = new Kinect(this);
    tracker = new KinectTracker();
    
    img = loadImage("Desktop/1.png");
...
//    drawImage(pg_obstacles); // use text as obstacle
    

    // sprite for fluid particles
    img_sprite = loadImage("/Users/samdief/Desktop/1.png");
...
//    drawImage(pg_obstacles); // use text as obstacle
  ...
  public void draw() {
    ...
    canvas.image(pg_fluid    , 0, 0);
    canvas.image(pg_obstacles, 0, 0);
